...
public class ExerciseFB {
	...
	public enum RampFilterType {
		NONE, RAMLAK, SHEPPLOGAN
	};
	...
	void initParameters() {
		...
		halfFanAngle = (float) Math.atan((detectorLength / 2.f) / focalLength); // DONE: determine the half fan angle
																				// such that the complete fan covers the
																				// detector exactly (hint: trigonometry)
		maxBeta = (float) Math.PI + 2.f * halfFanAngle; // DONE: compute the short scan minimum range using the value of
														// halfFanAngle
	...
	void doShortScanReconstruction() {
		...
		sino_cosW = applyCosineWeights(sino); // DONE: Go to this method and implement the missing steps!
		sino_cosW.show("After Cosine Weighting");

		// apply Parker redundancy weights
		sino_cosW_parkerW = applyParkerWeights(sino_cosW); // DONE: Go to this method and implement the missing steps!
		...
		Grid2D reconstruction = backproject(sino_cosW_parkerW_filt, recoSize, spacing); // TODO: Go to this method and
	...
	public Grid2D applyCosineWeights(Grid2D sino) {
		...
		for (int i = 0; i < detectorPixels; ++i) {
			float begin = -detectorPixels / 2.f * detectorSpacing + detectorSpacing / 2.f;

			float t = begin + i * detectorSpacing; // DONE: compute center of the current detector pixel (zero is center
													// of detector)
			float gamma = (float) Math.atan(t / focalLength);
			cosineKernel.setAtIndex(i, (float) Math.cos(gamma)); // DONE (hint: use t and focalLength)
		}
	...
	public Grid2D applyParkerWeights(Grid2D sino) {
		...
		for (int i = 0; i < detectorPixels; ++i) {
			...
			gamma = (float) Math
					.atan((i * detectorSpacing - detectorLength / 2.f + 0.5f * detectorSpacing) / focalLength);
			...
			for (int b = 0; b < numProjs; ++b) {
				...
				// https://dx.doi.org/10.1118/1.595078 )
				if (beta <= 2 * (delta - gamma)) {

					double tmp = Math.PI / 4.f * beta / (delta - gamma); // TODO
					float val = (float) (Math.sin(tmp) * Math.sin(tmp)); // TODO
					...
					double tmp = Math.PI / 4.f * (Math.PI + 2.f * delta - beta) / (delta + gamma); // (error in the
																									// original paper)
					float val = (float) (Math.sin(tmp) * Math.sin(tmp)); // TODO
	 ...
	 * Cosine, redundancy weighting and ramp filters need to be applied separately
	 * beforehand!
	 * Remark: This implementation requires a detector whose center coincides with
	...
	public Grid2D backproject(Grid2D sino, int[] recoSize, float[] spacing) {
		...
		result.setSpacing(new double[] { (double) spacing[0], (double) spacing[1] });

		for (int p = 0; p < numProjs; ++p) {
			float beta = (float) p * betaIncrement;
			float cosBeta = (float) Math.cos((double) beta);
			float sinBeta = (float) Math.sin((double) beta);
			PointND source = new PointND(
					new double[] { (double) (-focalLength * sinBeta), (double) (focalLength * cosBeta), 0.0 });
			float offset = -detectorLength / 2.0F;
			PointND detBorder = new PointND(
					new double[] { (double) (offset * cosBeta), (double) (offset * sinBeta), 0.0 });
			SimpleVector dirDet = detBorder.getAbstractVector().multipliedBy(-1.0);
			if (dirDet == null) {
				dirDet = new SimpleVector(3);
			}

			dirDet.normalizeL2();
			StraightLine detLine = new StraightLine(detBorder, dirDet);
			detLine.setDirection(detLine.getDirection().normalizedL2());
			Grid1D currProj = sino.getSubGrid(p);

			for (int x = 0; x < recoSize[0]; ++x) {
				float wx = this.getWorldCoordinate(x, recoSize[0], spacing[0]);

				for (int y = 0; y < recoSize[1]; ++y) {
					float wy = this.getWorldCoordinate(y, recoSize[1], spacing[1]);
					PointND reconstructionPointWorld = new PointND(new double[] { (double) wx, (double) wy, 0.0 });
					StraightLine projectionLine = new StraightLine(source, reconstructionPointWorld);
					if (projectionLine != null) {
						projectionLine.setDirection(projectionLine.getDirection().normalizedL2());
					}

					PointND detPixel = projectionLine.intersect(detLine);
					float valueTemp;
					if (detPixel != null) {
						float t = (float) SimpleOperators.multiplyInnerProd(detPixel.getAbstractVector(), dirDet)
								- offset;
						if (t < 0.0F || t > detectorLength) {
							continue;
						}

						float value = InterpolationOperators.interpolateLinear(currProj,
								(double) (t / detectorSpacing - 0.5F));
						float dWeight = this.distanceWeight(reconstructionPointWorld, beta);
						valueTemp = value / (dWeight * dWeight);
					} else {
						valueTemp = 0.0F;
					}

					result.addAtIndex(x, recoSize[1] - y - 1, valueTemp);
				}
			}
		}

		float normalizationFactor = (float) ((double) numProjs / Math.PI);
	...
	public float getWorldCoordinate(int imCoordinate, int dim, float spacing) {
		float wCoordinate = (imCoordinate - dim / 2.f + 0.5f) * spacing; // TODO
		return wCoordinate;
	}
	...
	public float distanceWeight(PointND reconstructionPointWorld, float beta) {
		...
		float phi = (float) (1.5707963267948966
				+ Math.atan2(reconstructionPointWorld.get(1), reconstructionPointWorld.get(0)));
		float U = (float) (((double) focalLength + (double) radius * Math.sin((double) (beta - phi)))
				/ (double) focalLength);
		return U;
	}
	...
	public ExerciseFB() {
		...
		sino = sino.getGridOperator().transpose(sino); // warning "Error in transpose" can be ignored (too strict
														// tolerance settings)
